...
class CircularQueue(Queue[T]):
    ...
    def __str__(self) -> str:
        """ 
        Print out all the elements from the stack
        param: self: CircularQueue object
        returns: A string that contains all the elements of the stack
        Complexity: Best case: O(n)
                    Worst case: O(n)
        """
        head = self.front # Copy the self.front value to head to represent the front of the queue
        if len(self)!=0:  # If the Queue is not empty
            lst = []      # Create an empty list
            for i in range(len(self)):       # Loop through the stack
                lst.append(self.array[head]) # Append each pokemon object in the stack into the list
                head = (head+1) % len(self.array) # Increase the value of head to move through the circular queue
            ret = ", ".join(list(map(str,lst)))   # Create the output string by using the .join() method but first
                                                  # Map each pokemon object in the lst to str to call their own str method
