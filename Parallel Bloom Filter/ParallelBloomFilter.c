...
int readFile(const char *filename, int fileLength, char ***pUniqueWordsList) {
    ...
    */
	...
	bool isUnique;						// Declare isUnique boolean to signify word is unique.
    ...
    #pragma omp parallel 
	{
		// Declare a private pointer to an array of unique words for each thread.
		char **pUniqueWordsPrivate = NULL;
		int countPrivate = 0;

		// Allocate enough memory for the private pointer pUniqueWords
		pUniqueWordsPrivate = (char**)malloc(fileLength * sizeof(char *));
		
		// Parallel loop for reading and processing words from the file. nowait to disabled waiting for critical section.
		...
		for(i = 0; i < fileLength; i++) {
			...
			if (fscanf(file, "%s", word) == 1) {
				...
				// Check if the word is unique within the private array.
				while ((j < countPrivate) && (isUnique)) {
					// If word is unique within the private array, set isUnique to false.
					if (strcmp(pUniqueWordsPrivate[j], word) == 0) {
						isUnique = false;
					}
					j++;	// Increment j by one.
				}
				
				// If the word is unique within the private array, store it in the private array.
				if (isUnique) {
					// Duplicate the word and store it.
					pUniqueWordsPrivate[countPrivate] = strdup(word);
					countPrivate++;		// Increment the private unique word count by one.
				}
			}
		}
		
		// Critical section to merge the unique words from private arrays into the shared global array.
		#pragma omp critical
		{
			// Loop through the private unique word array and compare them to shared global array.
			for(int i = 0; i < countPrivate; i++) {
				// Set default values for variables.
				bool isUnique = true;
				int j = 0;
				
				// Check if the word in each private array is unique within the shared global array.
				while ((j < count) && (isUnique)) {
					// If so, set isUnique to false.
					if (strcmp(pUniqueWords[j], pUniqueWordsPrivate[i]) == 0) {
						isUnique = false;
					}
					j++;	// Increment j by one.
				}
				
				// If the word is unique, store it in the shared global array.
				if (isUnique) {
					// Duplicate the word and store it.
					pUniqueWords[count] = strdup(pUniqueWordsPrivate[i]);
					count++;	// Increment the global unique word count by one.
				}
			}
			
			// Clean up memory allocated for private unique word array.
			for (i = 0; i < fileLength; i++) {
				free(pUniqueWordsPrivate[i]);
			}
			free(pUniqueWordsPrivate);
		}
	}
...
double query(const char* filename, int fileLength, int m, int n,int *bitarray, char **totalUnique) {
	/**
	...
	*/
	...
	int i, j;							// Declare loop counter i and j.
	double falsePositiveCount = 0.0;	// An integer value representing the number of false positives.

	// Parallel loop for reading and processing words from the file. reduction clause to combine all local falsePositiveCount
	// to one shared at end of parallel loop.
	#pragma omp parallel for private(found, hasFound, j, result, word) reduction(+:falsePositiveCount)
	for(i = 0; i < fileLength; i++) {
		// Read a word from the file.z
		if (fscanf(file1, "%s %d", word, &result) != EOF) {
			// Set default values for variables.
			hasFound = false;
			j = 0;
			...
			while ((j < n) && (!hasFound)) {
				...
				if (strcmp(word, totalUnique[j]) == 0) {
					hasFound = true;
				}

				j++;	// Increment i by one to continue looping.
			}
...
int main() {
	...
	falsePositiveCount = query(queryName, queryLength, m, n, bitarray, totalUnique);
