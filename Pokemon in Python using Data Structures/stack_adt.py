...
class ArrayStack(Stack[T]):
    ...
    def __str__(self) -> str:
        ...
        param: self: ArrayStack object
        returns: A string that contains all the elements of the stack
        Complexity: Best case: O(n)
                    Worst case: O(n)
        """
        if len(self) != 0: # If the stack is not empty
            lst = self.array[len(self)-1::-1]   # Slice the array of the stack so it returns the reversed order
