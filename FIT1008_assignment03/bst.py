...
class BinarySearchTree(Generic[K, I]):
    ...
    def get_tree_node_by_key(self, key: K) -> TreeNode:
        """
            Attempts to get tree node by calling get_tree_node_by_key_aux, called by __getitem__
            :complexity best: O(CompK) finds the item in the root of the tree
            :complexity worst: O(CompK * D) item is not found, where D is the depth of the tree
            CompK is the complexity of comparing the keys
        """
    ...
    def get_tree_node_by_key_aux(self, current: TreeNode, key: K) -> TreeNode:
        """
            Attempts to get tree node by key, called by get_tree_node_by_key
            :complexity best: O(CompK) finds the item in the root of the tree
            :complexity worst: O(CompK * D) item is not found, where D is the depth of the tree
            CompK is the complexity of comparing the keys
        """
    ...
    def __setitem__(self, key: K, item: I) -> None:
        """
            Attempts to set item for the bst by inserting the input key to the bst with insert_aux
    ...
    def delete_aux(self, current: TreeNode, key: K) -> TreeNode:
        """
            ...
            determine the node to delete. And find the successor of the deleted node And link it with predecessor of
            the current
            :complexity best: O(CompK) finds the item in the root of the tree and item is a leaf
            :complexity worst: O(log n)  where n is the number of node in the tree if it's root
        """
    ...
    def get_successor(self, current: TreeNode) -> TreeNode | None:
        ...
        larger keys.
        :params: self: A BinarySearchTree class object
                 current: A TreeNode object
        :pre: current has to be a TreeNode object
        :raises TypeError: if current is not a TreeNode object
        :complexity: Best Case: O(1)
                   : Worst Case: O(log n) where n is the number of node in the tree
        """
        # If current is not a TreeNode object, raise TypeError
        if not isinstance(current, TreeNode):
            raise TypeError("current has to be a TreeNode object")

        # if there is no right node for the current then there is no successor
        if current.right is None:
            return None
        else:
            return self.get_minimal(current.right)  # try to get the successor of the current by searching for the minimal
            # node of from the current.right subtree

    def get_predecessor(self, current: TreeNode) -> TreeNode:
        """
        Get predecessor of the current node.
        It should be a child node having the largest key among all the
        smaller keys.
        :params: self: A BinarySearchTree class object
                 current: A TreeNode object
        :pre: current has to be a TreeNode object
        ...
        # if there is no right node for the current then there is no predecessor
        if current.left is None:
            return None
        else:
            node = current.left # start at the left subtree of current
            # while the node is not a leaf or it still has a right node(not the predecessor node yet)
            while not self.is_leaf(node) and not node.right is None:
                node = node.right # the predecessor is in the right subtree

            return node # the node is the predecessor node of current so return it
    ...
    def get_minimal(self, current: TreeNode) -> TreeNode:
        ...
        # while the node is not a leaf or it still has a left node(not the minimal node yet)
        while not self.is_leaf(current) and not current.left is None:
            return self.get_minimal(current.left)  # the minimal node is in the left subtree

        return current  # the node is the minimal node in the subtree
